...
func (ts *MFATestSuite) TestEnrollFactor() {
	...
	for _, c := range cases {
		ts.Run(c.desc, func() {
			...
			token, err := generateAccessToken(ts.API.db, user, nil, time.Second*time.Duration(ts.Config.JWT.Exp), ts.Config)
			require.NoError(ts.T(), err)
...
func (ts *MFATestSuite) TestChallengeFactor() {
	...
	token, err := generateAccessToken(ts.API.db, u, nil, time.Second*time.Duration(ts.Config.JWT.Exp), ts.Config)
...
func (ts *MFATestSuite) TestMFAVerifyFactor() {
	...
	for _, v := range cases {
		ts.Run(v.desc, func() {
			...
			token, err := generateAccessToken(ts.API.db, user, r.SessionId, time.Second*time.Duration(ts.Config.JWT.Exp), ts.Config)
...
func (ts *MFATestSuite) TestUnenrollVerifiedFactor() {
	...
	for _, v := range cases {
		...
		ts.Run(v.desc, func() {
			...
			token, err := generateAccessToken(ts.API.db, u, &s.ID, time.Second*time.Duration(ts.Config.JWT.Exp), ts.Config)
			require.NoError(ts.T(), err)
